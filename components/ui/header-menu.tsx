...
const HeaderMenue = ({className}:{className?:string}) => {
  ...
  return (
    <div className={cn(" md:inline-flex justify-between gap-6 font-semibold text-white inline-block w-1/3 border-shadow-lg ",className)}>
      {HeaderData?.map((item)=>(
        <Link
         key={item?.title} href={item?.href} className={`hover:text-logo-primary hoverEffect relative group ${pathname === item?.href && "text-logo-primary"}`}>
            {item?.title}
            <span
